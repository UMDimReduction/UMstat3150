...
There is a different approach to estimating the CDF of any distribution, as long as we can generate random variates. The idea is as follows: assume we want to estimate $F(x) = P(X \leq x)$. Sample $n$ variates from $F$ and count the proportion of samples that are at most $x$. Easy peasy!
...
$$\mathrm{Var}(I(X \leq x)) = \Phi(x) (1 - \Phi(x)).$$
